...
string Base64decode(const string val) {
    size_t outSize = BASE64_DECODE_OUT_SIZE(val.length());
    char *tmp = (char *)malloc(outSize);
    base64_decode(val.c_str(), val.length(), (uint8_t*)tmp);
    return string(tmp);
}

string Base64encodeUint8(uint8_t *val, uint32_t len) {
    size_t outSize = BASE64_ENCODE_OUT_SIZE(len);
    char *tmp = (char *)malloc(outSize);
    base64_encode((const uint8_t *)val, len, tmp);
    return string(tmp);
