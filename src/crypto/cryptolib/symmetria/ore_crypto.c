...
int generate_aes_key(ORE_AES_KEY *key)
{
  ...
  char *tmp = getAESKey_c();
  mymemcpy(keybuf, tmp, AES_KEY_BYTES);
  free(tmp);
...
static int _feistel_round_batch(uint64_t *dst_l, uint64_t *dst_r, uint32_t nblocks,
                                ...
                                uint32_t nbits)
{
  mymemcpy(dst_l, src_r, nblocks * sizeof(uint64_t));
...
static int _prp_eval_all_helper(uint64_t *dst, const ORE_AES_KEY *key, uint32_t nbits, bool inv)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    mymemcpy(val_l, val_l_new, nblocks * sizeof(uint64_t));
    mymemcpy(val_r, val_r_new, nblocks * sizeof(uint64_t));
  }
