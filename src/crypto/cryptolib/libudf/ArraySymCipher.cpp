...
hash_map<int, long long> ArraySymCipher::getcardPos()
{
    return this->cardPos;
}
hash_map<int, long long> ArraySymCipher::getcardNeg()
{
    return this->cardNeg;
}
...
void ArraySymCipher::addIds(ArraySymCipher other, bool isPos)
{
    ...
    hash_map<int, long long> thisCard;
    ...
    hash_map<int, long long> otherCard;
    ...
    hash_map<int, long long> newCard;
...
void ArraySymCipher::multiplyIds(long long multiplier, long long modulo)
{
    ...
    if (multiplier < 0) {
        ...
        // swap cardinalities
        hash_map<int, long long> card = cardPos;
        cardPos = cardNeg;
        cardNeg = card;
    }
    ...
    hash_map<int, long long>::iterator iter;
    iter = cardPos.begin();
    if (!cardPos.empty())
    ...
    cout << "offsetPOS: " << offsetPos << endl;
    cout << "cardmultiplier: " << cardMultiplierPos << endl;
    hash_map<int, long long>::iterator itera;
    for (itera = cardPos.begin(); itera != cardPos.end(); itera++) {
        cout << "cardPOS_key: " << itera->first << " cardPOS_VALUE: " << itera->second << endl;
    }
    for (int b = 0; b < idsNeg.size(); b++) {
        cout << "idsNegarray: " << idsNeg[b] << endl;
    }
    ...
    cout << "offsetNEG: " << offsetNeg << endl;
    cout << "cardmultiplierNEG: " << cardMultiplierNeg << endl;
    hash_map<int, long long>::iterator iterb;
    for (iterb = cardNeg.begin(); iterb != cardNeg.end(); iterb++) {
        cout << "cardNEG_key: " << iterb->first << " cardNEG_VALUE: " << iterb->second << endl;
    }
}
