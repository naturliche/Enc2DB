...
sgx_status_t MessageHandler::initEnclave() {
    this->enclave = EnclaveManager::getInstance();
    //TODO: 是否开启RA没有分辨
    // this->enclave->useRA();
...
void MessageHandler::assembleMSG2(RAMessages::MessageMSG2 msg, sgx_ra_msg2_t **pp_msg2) {
    ...
    memcpy(&p_msg2->sig_rl, &sigrl, p_msg2->sig_rl_size);

    *pp_msg2 = p_msg2;
}


string MessageHandler::handleMSG2(RAMessages::MessageMSG2 msg) {
    Log("Received MSG2");
...
void MessageHandler::assembleAttestationMSG(RAMessages::AttestationMessage msg, ra_samp_response_header_t **pp_att_msg) {
...
string MessageHandler::handleAttestationResult(RAMessages::AttestationMessage msg) {
...
string MessageHandler::handleMSG0(RAMessages::MessageMsg0 msg) {
...
string MessageHandler::createInitMsg(int type, string msg) {
    RAMessages::SecretMessage init_msg;
...
vector<string> MessageHandler::incomingHandler(string v, int type)
{
    ...
    { //Verification request
        RAMessages::InitialMessage init_msg;
    ...
    case RA_ATT_RESULT:
    { //Reply to MSG3
        RAMessages::AttestationMessage att_msg;
